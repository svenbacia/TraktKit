...
class TraktExploreTests: XCTestCase {
    ...
    func testPopularShows() {
        ...
        let task = trakt.load(resource: trakt.resources.explore.shows.popular(), authenticated: false) { (result) in
            if result.value?.0.count == 10 {
                expectation.fulfill()
            } else {
                XCTFail()
    ...
    func testAnticipatedShows() {
        ...
        let task = trakt.load(resource: trakt.resources.explore.shows.anticipated(), authenticated: false) { (result) in
            if result.value?.0.count == 10 {
                expectation.fulfill()
            } else {
                XCTFail()
    ...
    func testPlayedShows() {
        ...
        let task = trakt.load(resource: trakt.resources.explore.shows.played(), authenticated: false) { (result) in
            if result.value?.0.count == 10 {
                expectation.fulfill()
            } else {
                XCTFail()
    ...
    func testWatchedShows() {
        ...
        let task = trakt.load(resource: trakt.resources.explore.shows.watched(), authenticated: false) { (result) in
            if result.value?.0.count == 10 {
                expectation.fulfill()
            } else {
                XCTFail()
    ...
    func testCollectedShows() {
        ...
        let task = trakt.load(resource: trakt.resources.explore.shows.collected(), authenticated: false) { (result) in
            if result.value?.0.count == 10 {
                expectation.fulfill()
            } else {
                XCTFail()
